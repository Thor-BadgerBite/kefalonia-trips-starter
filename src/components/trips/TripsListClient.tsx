...
export default function TripsListClient({ trips: initialTrips, created, updated, deleted }: TripsListClientProps) {
  ...
  useEffect(() => {
    if (created) {
      ...
      setNotificationMessage('Trip updated successfully!');
      setNotificationType('success');
      setShowNotification(true);
      setTimeout(() => {
        setShowNotification(false);
      ...
      setNotificationMessage('Trip deleted successfully!');
      setNotificationType('success');
      setShowNotification(true);
      setTimeout(() => {
        setShowNotification(false);
  ...
  }, [created, updated, deleted, router]);

  const handleToggleActive = async (tripId: string, currentStatus: boolean) => {
    try {
      ...
      setTrips(trips.map(trip =>
        trip.id === tripId ? { ...trip, active: !currentStatus } : trip
      ));

      setNotificationMessage(currentStatus ? 'Trip deactivated' : 'Trip activated');
      setNotificationType('success');
      setShowNotification(true);
      setTimeout(() => setShowNotification(false), 2000);
    } catch (error) {
      console.error('Error toggling trip status:', error);
      setNotificationMessage('Failed to update trip status');
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        onClick={() => handleToggleActive(trip.id, trip.active)}
